    ...
    return [
        {
            ...
            description: "Structured Differential review prompt that recursively resolves referenced tasks/revisions and returns JSON findings.",
            arguments: [
                {
                    name: "revision_id",
...
export function getPromptByName(name, args) {
    if (name !== REVIEW_PROMPT_NAME) {
        throw new Error(`Unknown prompt: ${name}`);
    }
    const revisionId = normalizeRevisionId(args.revision_id);
    return {
        description: "Review a Differential by building recursive task/revision context and output only JSON findings.",
...
function buildRecursiveReviewPrompt(revisionId) {
    return [
