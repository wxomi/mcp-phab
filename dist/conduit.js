...
import { normalizeConduitResponse } from "./parsing.js";
...
export async function callConduit(method, payload, options = {}) {
    if (!method.trim()) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            timedOut = true;
            child.kill("SIGKILL");
            reject(new ArcTimeoutError(`arc call-conduit timed out after ${timeoutMs}ms for method '${method}'.`));
        }, timeoutMs);
        child.on("error", (error) => {
            clearTimeout(timer);
            const code = error.code;
            if (code === "ENOENT") {
                reject(new ArcNotFoundError("Could not find 'arc' on PATH. Install Arcanist and ensure it is configured and authenticated."));
                return;
            }
        ...
        child.on("close", (code) => {
            ...
            try {
                ...
                reject(new ConduitResponseError(`arc returned invalid JSON for method '${method}': ${error.message}. Output: ${preview}`));
                return;
            }
            try {
        ...
        try {
            ...
            clearTimeout(timer);
            reject(new ArcExecutionError(`Failed writing JSON payload to arc stdin: ${error.message}`));
        }
    });
}
