...
function buildRecursiveReviewPrompt(): string {
  return [
    ...
    "",
    "Review requirements:",
    "- Treat the Differential raw diff as the source of truth for what changed and for the post-patch code being reviewed.",
    ...
    "- Validate that every newly introduced symbol/constant/method used in changed lines exists and is valid in the post-patch code implied by the diff and surrounding provided context.",
    "- Flag runtime/syntax/name errors before higher-level logic issues.",
    "- You must explicitly review for increased primary DB load.",
    "- You must explicitly review for increased query count or query cost.",
    "- You must explicitly review for N+1 query patterns.",
    "- You must explicitly review for changes that move reads from replica to primary.",
    "- You must explicitly review for synchronous execution replacing deferred or asynchronous execution.",
    "- You must explicitly review for batch jobs or Celery paths that now perform more work per item.",
    "- You must explicitly review for changes that are safe for small datasets but unsafe at production scale.",
    "- You must explicitly review for connection pressure, lock amplification, and queue backlog risk.",
    ...
    "- Assume review happens before the patch lands. Local workspace files may still reflect the pre-patch state.",
    "- Do not report that the workspace file differs from the diff as a bug or mismatch by itself.",
    "- If you consult local file contents for extra context, mentally apply the diff first and reason about the resulting post-patch code, not the current checked-out file alone.",
    "- Only raise an issue related to surrounding code or symbol resolution when it would still be a real problem after applying the provided diff.",
    "",
